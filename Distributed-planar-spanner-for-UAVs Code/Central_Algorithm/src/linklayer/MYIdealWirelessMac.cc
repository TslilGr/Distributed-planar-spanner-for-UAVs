...
Define_Module(MYIdealWirelessMac);

simsignal_t MYIdealWirelessMac::radioStateSignal = registerSignal("radioState");
simsignal_t MYIdealWirelessMac::dropPkNotForUsSignal = registerSignal("dropPkNotForUs");


MYIdealWirelessMac::MYIdealWirelessMac()
{
    queueModule = NULL;
    radioModule = NULL;
}
...
void MYIdealWirelessMac::flushQueue()
{
    ASSERT(queueModule);
...
void MYIdealWirelessMac::clearQueue()
{
    ASSERT(queueModule);
    queueModule->clear();
}

void MYIdealWirelessMac::initialize(int stage)
{
    ...
    if (stage == 0)
    {
        ...
        EV << "MYIdealWirelessMac::radioModule check !!!!! 4 SUBSCRIBE" << radioModule << endl;
        MYIdealRadio *irm = check_and_cast<MYIdealRadio *>(radioModule);
...
void MYIdealWirelessMac::initializeMACAddress()
{
...
InterfaceEntry *MYIdealWirelessMac::createInterfaceEntry()
{
...
void MYIdealWirelessMac::receiveSignal(cComponent *src, simsignal_t id, long x)
{
...
void MYIdealWirelessMac::startTransmitting(cPacket *msg)
{
...
void MYIdealWirelessMac::handleSelfMsg(cMessage *msg)
{
    error("Unexpected self-message");
}

void MYIdealWirelessMac::getNextMsgFromHL()
{
...
void MYIdealWirelessMac::handleUpperMsg(cPacket *msg)
{
    outStandingRequests--;
    if (radioState == RadioState::TRANSMIT)
    {
...
void MYIdealWirelessMac::handleCommand(cMessage *msg)
{
    error("Unexpected command received from higher layer");
}

void MYIdealWirelessMac::handleLowerMsg(cPacket *msg)
{
...
IdealWirelessFrame *MYIdealWirelessMac::encapsulate(cPacket *msg)
{
...
bool MYIdealWirelessMac::dropFrameNotForUs(IdealWirelessFrame *frame)
{
...
cPacket *MYIdealWirelessMac::decapsulate(IdealWirelessFrame *frame)
{
    // decapsulate and attach control info
