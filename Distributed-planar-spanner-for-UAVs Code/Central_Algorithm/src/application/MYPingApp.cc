...
Define_Module(MYPingApp);

simsignal_t MYPingApp::rttSignal = registerSignal("rtt");
simsignal_t MYPingApp::numLostSignal = registerSignal("numLost");
simsignal_t MYPingApp::outOfOrderArrivalsSignal = registerSignal("outOfOrderArrivals");
simsignal_t MYPingApp::pingTxSeqSignal = registerSignal("pingTxSeq");
simsignal_t MYPingApp::pingRxSeqSignal = registerSignal("pingRxSeq");
//MOSHE: added signal
simsignal_t MYPingApp::extraDroneToCCSignal = registerSignal("extraDroneToCC");
...
void MYPingApp::initialize(int stage)
{
...
void MYPingApp::handleMessage(cMessage *msg)
{
    if (!isNodeUp())
    {
...
bool MYPingApp::handleOperationStage(LifecycleOperation *operation, int stage, IDoneCallback *doneCallback)
{
    Enter_Method_Silent();
...
void MYPingApp::startSendingPingRequests()
{
...
void MYPingApp::stopSendingPingRequests()
{
    pid = -1;
    lastStart = -1;
    sendSeqNo = expectedReplySeqNo = 0;
...
void MYPingApp::scheduleNextPingRequest(simtime_t previous)
{
    simtime_t next;
    if (previous == -1)
...
void MYPingApp::sendPingRequest()
{
    ...
    //XXXXXXX should be set as the destAddr after 5 pings, drone[1] should not send to self
    if(sendSeqNo>2 ){
        EV << "----------------- MYPingApp::sendPingRequest MY DEST ALL dests should be host[0] ----------------- \n";
        //now its an ERROR but when making a linker var, that would not happen (linker never self)
        destAddr = IPvXAddressResolver().resolve("host[0]");
    }
...
void MYPingApp::sendToICMP(cMessage *msg, const IPvXAddress& destAddr, const IPvXAddress& srcAddr, int hopLimit)
{
    if (!destAddr.isIPv6())
    {
        // send to IPv4
...
void MYPingApp::processPingResponse(PingPayload *msg)
{
...
void MYPingApp::countPingResponse(int bytes, long seqNo, simtime_t rtt)
{
...
void MYPingApp::receiveSignal(cComponent *source, simsignal_t signalID, long hostNum)
{
    EV_INFO << "CCCCCCCCCCCCCCCCCC222 MYPingApp::receiveSignal, signalID = " << signalID << " CCCCCCCCCCCCCCCCCC222" <<endl;
    //if (signalID == extraDroneToCCSignal){
    //    EV_INFO << "CCCCCCCCCCCCCCCCCC222 MYIdealChannelModelAccess::receiveSignal extraDroneToCCSignal the hostNum is: "<< hostNum <<" CCCCCCCCCCCCCCCCCC222 "<< endl;
        //now we want to use the signal to send a msg with the new function MYIdealChannelModel::sendToMYChannel and see if its possible to send msg this way (using direct)
        //also we need to make sure that the correct msg and airframe are build

    //    if (myRadioRef)
    //        cc->sendToMYDroneChannelTEST(myRadioRef, hostNum);
